...
const AadhaarExtractor = () => {
    ...
    const handleExtract = async () => {
        ...
        try {
            ...
            // Mapping backend response (M/F) to frontend state (male/female)
    ...
    const handlePhotoReplace = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                // Base64 string extract karke state update karna
                const base64String = reader.result.split(',')[1];
                setData({ ...data, photo_base64: base64String });
            };
    ...
    const handleGenerate = async () => {
        try {
            ...
            alert("Generation failed. Check balance.");
        } finally {
            setGenLoading(false);
        }
    };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-50">
                            ...
                            <label
                                htmlFor="file-input"
                                className="cursor-pointer block border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center bg-slate-50"
                            >
                            ...
                            <button onClick={handleExtract}
                                className="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-2xl shadow-lg">
                                {loading ? "⌛ EXTRACTING..." : "⚡ START EXTRACTION"}
                            </button>
                        </div>

                        <div className="bg-slate-900 p-6 rounded-3xl shadow-xl text-white">
                            <h3 className="font-bold mb-4">💳 PAYMENT & PRINT</h3>
                            <select value={paymentMethod}
                                onChange={(e) => setPaymentMethod(e.target.value)}
                                className="w-full bg-slate-800 rounded-xl py-3 px-4 mb-4">
                                <option value="wallet">💰 Wallet (₹20)</option>
                                <option value="cashfree">🚀 Cashfree Gateway</option>
                            </select>

                            <button onClick={handleGenerate}
                                className="w-full bg-green-500 hover:bg-green-400 text-white font-black py-4 rounded-2xl shadow-lg">
